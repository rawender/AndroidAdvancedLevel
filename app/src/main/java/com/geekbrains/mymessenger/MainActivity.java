...
public class MainActivity extends AppCompatActivity {
        ...
        if(requestCode == permissionRequestCode) {
            if(grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(getApplicationContext(),
                        "Sorry, the application may not work properly without this permission",
    ...
    private void sendSms() {
        ...
        if (phoneNumber.equals("") || checkPhoneNumber(phoneNumber)) {
            Toast.makeText(getApplicationContext(), "The phone number is not listed", Toast.LENGTH_LONG).show();
        } else if (msg.equals("")) {
            Toast.makeText(getApplicationContext(), "You can't send blank message", Toast.LENGTH_LONG).show();
        } else {
